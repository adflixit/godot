...
void TrailMesh::surface_begin(PrimitiveType p_primitive, const Ref<Material> &p_material) {
...
void TrailMesh::surface_set_color(const Color &p_color) {
...
void TrailMesh::surface_set_normal(const Vector3 &p_normal) {
	ERR_FAIL_COND_MSG(!surface_active, "Not creating any surface. Use surface_begin() to do it.");
...
void TrailMesh::surface_set_tangent(const Plane &p_tangent) {
	ERR_FAIL_COND_MSG(!surface_active, "Not creating any surface. Use surface_begin() to do it.");
	if (!uses_tangents) {
...
void TrailMesh::surface_set_uv(const Vector2 &p_uv) {
	ERR_FAIL_COND_MSG(!surface_active, "Not creating any surface. Use surface_begin() to do it.");
	if (!uses_uvs) {
...
void TrailMesh::surface_set_uv2(const Vector2 &p_uv2) {
	ERR_FAIL_COND_MSG(!surface_active, "Not creating any surface. Use surface_begin() to do it.");
	if (!uses_uv2s) {
...
void TrailMesh::surface_add_vertex(const Vector3 &p_vertex) {
	ERR_FAIL_COND_MSG(!surface_active, "Not creating any surface. Use surface_begin() to do it.");
...
void TrailMesh::surface_add_vertex_2d(const Vector2 &p_vertex) {
	ERR_FAIL_COND_MSG(!surface_active, "Not creating any surface. Use surface_begin() to do it.");
	ERR_FAIL_COND_MSG(vertices.size() && !active_surface_data.vertex_2d, "Can't mix 2D and 3D vertices in a surface.");
...
int TrailMesh::get_surface_count() const {
	return surfaces.size();
}

int TrailMesh::surface_get_array_len(int p_idx) const {
	ERR_FAIL_INDEX_V(p_idx, int(surfaces.size()), -1);
	return surfaces[p_idx].array_len;
}

int TrailMesh::surface_get_array_index_len(int p_idx) const {
	return 0;
}

Array TrailMesh::surface_get_arrays(int p_surface) const {
	ERR_FAIL_INDEX_V(p_surface, int(surfaces.size()), Array());
	return RS::get_singleton()->mesh_surface_get_arrays(mesh, p_surface);
}

TypedArray<Array> TrailMesh::surface_get_blend_shape_arrays(int p_surface) const {
	return TypedArray<Array>();
}

Dictionary TrailMesh::surface_get_lods(int p_surface) const {
	return Dictionary();
}

BitField<Mesh::ArrayFormat> TrailMesh::surface_get_format(int p_idx) const {
	ERR_FAIL_INDEX_V(p_idx, int(surfaces.size()), 0);
	return surfaces[p_idx].format;
}

Mesh::PrimitiveType TrailMesh::surface_get_primitive_type(int p_idx) const {
	ERR_FAIL_INDEX_V(p_idx, int(surfaces.size()), PRIMITIVE_MAX);
	return surfaces[p_idx].primitive;
}

void TrailMesh::surface_set_material(int p_idx, const Ref<Material> &p_material) {
...
Ref<Material> TrailMesh::surface_get_material(int p_idx) const {
	ERR_FAIL_INDEX_V(p_idx, int(surfaces.size()), Ref<Material>());
	return surfaces[p_idx].material;
}

int TrailMesh::get_blend_shape_count() const {
	return 0;
}

StringName TrailMesh::get_blend_shape_name(int p_index) const {
	return StringName();
}

void TrailMesh::set_blend_shape_name(int p_index, const StringName &p_name) {
}

AABB TrailMesh::get_aabb() const {
	AABB aabb;
...
void TrailMesh::_bind_methods() {
	ClassDB::bind_method(D_METHOD("surface_begin", "primitive", "material"), &TrailMesh::surface_begin, DEFVAL(Ref<Material>()));
	ClassDB::bind_method(D_METHOD("surface_set_color", "color"), &TrailMesh::surface_set_color);
	ClassDB::bind_method(D_METHOD("surface_set_normal", "normal"), &TrailMesh::surface_set_normal);
	ClassDB::bind_method(D_METHOD("surface_set_tangent", "tangent"), &TrailMesh::surface_set_tangent);
	ClassDB::bind_method(D_METHOD("surface_set_uv", "uv"), &TrailMesh::surface_set_uv);
	ClassDB::bind_method(D_METHOD("surface_set_uv2", "uv2"), &TrailMesh::surface_set_uv2);
	ClassDB::bind_method(D_METHOD("surface_add_vertex", "vertex"), &TrailMesh::surface_add_vertex);
	ClassDB::bind_method(D_METHOD("surface_add_vertex_2d", "vertex"), &TrailMesh::surface_add_vertex_2d);
	ClassDB::bind_method(D_METHOD("surface_end"), &TrailMesh::surface_end);

	ClassDB::bind_method(D_METHOD("clear_surfaces"), &TrailMesh::clear_surfaces);
}

RID TrailMesh::get_rid() const {
	return mesh;
}
